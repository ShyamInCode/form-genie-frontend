...
const Template = ({ setFormName }) => {
  ...
  const [customClientSecret, setCustomClientSecret] = useState(
    URLSearchParams.get("clientsecret")
  );
  ...
  const [heading, setHeading] = useState("Sign Up For Email Updates!");
  const [submitButtonText, setSubmitButtonText] = useState(
    "Submit your details"
  );
  const [submitButtonUrl, setSubmitButtonUrl] = useState(""); // State added for button url
  const [selectedFontOne, setSelectedFontOne] = useState("Lato"); // Font selection state
  ...
  const [fntColor, setFntColor] = useState("#000000");
  const [inputLabelColor, setInputColor] = useState("#000000");
  const [btnColor, setBtnColor] = useState("#ff3951");
  const [btnFntColor, setBtnFntColor] = useState("#ffffff");
  ...
  // function handleCheckboxClick(index, newRequiredState) {
  //   const updatedFirstGroup = [...firstGroup];
  //   updatedFirstGroup[index].required = !updatedFirstGroup[index].required;
  //   setFirstGroup(updatedFirstGroup);
  // }

  // function handleDefaultValueChange(index, newDefaultValue) {
  //   const updatedFirstGroup = [...firstGroup];
  //   updatedFirstGroup[index].defaultValue = newDefaultValue;
  //   setFirstGroup(updatedFirstGroup);
  // }
  ...
  ///////////////////////////////////////////////
  const [isFiredEntryEvent, setIsFiredEntryEvent] = useState(false);
  ...
  async function submitForm(fileURL) {
    ...
    //window.localStorage.setItem('customCid', customClientId);
    //window.localStorage.setItem('customCSec', customClientSecret);
    //window.localStorage.setItem('customSubd', customSubdomain);
    //window.localStorage.setItem('customCid', customClientId);
    //window.localStorage.setItem('customCSec', customClientSecret);
    //window.localStorage.setItem('customSubd', customSubdomain);
    // if(customSubdomain && customClientId) {
    //     window.open(`https://${customSubdomain}.auth.marketingcloudapis.com/v2/authorize?response_type=code&client_id=${customClientId}&redirect_uri=https://c313-139-135-32-171.ngrok-free.app/redirect`, '_self');
    //     window.localStorage.setItem('customCid', customClientId);
    //     window.localStorage.setItem('customCSec', customClientSecret);
    //     window.localStorage.setItem('customSubd', customSubdomain);
    //     return
    ...
    // if (submitButtonUrl) {
    //   // Redirect users to the designated URL
    //   window.location.href = submitButtonUrl;
    //   return; // Exit the function to prevent further steps
    // }

    window.open(
      `${Subdomain}v2/authorize?response_type=code&client_id=${ClientId}&redirect_uri=${window.location.origin}/redirect`,
      "_self"
    );
  }
            ...
            <div className="SelectionBox">
              ...
              >
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  ...
                  {/* <FormField state={logo} setState={setLogo} className={"label-style"}
                                               name={'Add your logo'} id={'btn-Fnt-color'} type={'file'}/> */}
                </div>
                <div style={{ display: "flex", alignItems: "center" }}>
                  <div className="subTitleShape" />
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  ...
                  {isFiredEntryEvent && (
                    <>
                      {/* <Flex direction="column">
                        <input
                          value={firedEntryEmail}
                          type="email"
                          className="text-field2"
                          placeholder={`Enter Email`}
                          onChange={(event) => {
                            setFiredEmail(event.target.value);
                          }}
                        />
                        </Flex> */}
                      <Flex direction="column" mt={2}>
          ...
          >
            ...
            >
              <div style={previewFormStyles}>
                  ...
                  >
                    ...
                    <div className="input-control">
                      <button
                        ...
                        className="submit-btn" /*onClick={(e) => {
                        e.preventDefault(); // Prevent default form submission behavior  if (submitButtonUrl) {  window.location.href = submitButtonUrl; // Redirect if submitButtonUrl is provided
                        } else {
                          // default submit form logic here
                        }
                      } }*/
                      >
                        {submitButtonText}
                ...
                {/* Render your preview form components here */}
              </div>
            </div>
          </div>
        </div>
